...
string memory lalo,string memory name  ,uint256 pieceamount ,string memory url

task("createLandMark", "MintWithCondition for user")
    .addParam("contractaddress", "The Factory address")
    .addParam("lalo", "latitudes,")
    .addParam("name","")
    .addParam("pieceamount", "The account to send to")
    .addParam("url", "The account to send to")
    .setAction(async (taskArgs) => {
        const contractAddr = taskArgs.contractaddress
        const user = taskArgs.lalo
        const name = taskArgs.name
        const pieceamount = taskArgs.pieceamount
        ...
        console.log("Sending:", amount," from address:",signer.address , "AP to", contractAddr)


        try {
            result = await simpleCoinContract.createLandMark(user,amount,point, {
                gasLimit: 1000000000,
